...
wget http://10.20.3.48/download/XGEN-UTX-RB/build_logs/$BAMBOO_buildResultKey.log
...
echo $start_line2
echo $end_line2
awk "NR>=$start_line1&&NR<=$end_line1" temp1.log >jobAcknowledge.xml
awk "NR>=$start_line2&&NR<=$end_line2" temp1.log >>jobAcknowledge.xml
rm -fr XGEN-UTX-RB-*
rm -fr temp*.log
...
cat jobAcknowledge.xml
grep "in progress" jobAcknowledge.xml
if [ $? -eq 0 ]; then
    echo "====================================="
    echo "At least one API/VLIB rebuild failed."
    echo "====================================="
    exit -1
else
    echo "======================================================"
